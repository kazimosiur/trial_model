...
customers_dedup AS (
  SELECT
    *
    , ROW_NUMBER() over (PARTITION BY global_entity_id, analytical_customer_id) as row_n
...
qc_agg AS (
  SELECT
    ...
    ,COALESCE(orders.darkstores.lifetime.order_count,0) + COALESCE(orders.localstores.lifetime.order_count,0)        AS qc_lifetime_order_count_qc
    ,COALESCE(orders.darkstores.lifetime.order_count,0)                                                              AS qc_lifetime_order_count_dmart
    ,COALESCE(orders.localstores.lifetime.order_count,0)                                                             AS qc_lifetime_order_count_ls
...
order_data AS (
  SELECT
    ...
    ,agg.qc_lifetime_order_count_qc                       AS order_count_lifetime_vert_qc
    ,agg.qc_lifetime_order_count_dmart                    AS order_count_lifetime_vert_dmart
--     ,agg.qc_lifetime_order_count_ls                       AS order_count_lifetime_vert_ls
   ...
   ------------------------------------------------------------
    ...
    ------------------------------------------------------------
     ...
     ---adding local store features---
    ,cust.orders.localstores.lifetime.deliv_dist_m_avg         AS deliv_dist_m_avg_lifetime_vert_ls
    ,cust.orders.localstores.last4w.deliv_dist_m_avg           AS deliv_dist_m_avg_l04w_vert_ls
    ,cust.orders.localstores.last16w.deliv_dist_m_avg          AS deliv_dist_m_avg_l16w_vert_ls
    ---
    ...
    ,cust.location.coverage_metrics.dist_nearest_localstore  AS dist_nearest_localstore_vert_ls

  FROM customers_dedup as cust
  ...
  WHERE 
    (
    orders.restaurants.first_order.order_date < CURRENT_DATE() - 1 -  90      # allow 2 weeks between first order and possible QC order
    AND orders.restaurants.last_order.order_date > CURRENT_DATE() - 1 - 270   # any order in last 180d
    )
    AND
    (
        orders.restaurants.first_order.order_date < orders.restaurants.last_order.order_date
        AND orders.restaurants.last_order.order_date < orders.darkstores.first_order.order_date 
    )
    OR 
    (
        orders.darkstores.first_order.order_date is NULL # adding customers before they ordered their first Dmart orders
    )
)
...
--   FROM       `dh-darkstores-stg.dev_dmart.qc_trail_front_end_workaround`        AS p
--   INNER JOIN `fulfillment-dwh-production.curated_data_shared_mkt.braze_external_id_map` AS map
...
,qubik_agg AS (
   ...
   LEFT JOIN `dh-darkstores-stg.dev_dmart.qc_trail_front_end_workaround` AS qub 
                                                                   ON  qc.global_entity_id = qub.global_entity_id
...
--   AND coverage_vendors_qc>0
  ...
  AND global_entity_id ='OP_SE' -- 'OP_SE'
